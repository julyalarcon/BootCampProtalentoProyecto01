...
public class UsuarioImp implements IUsuario {
	...
	public Usuario buscarPorCorreoClave(String correo, String clave) {
		Usuario usuario = null;
		String sql = "select correo, fechaCreacion, fechaUltimoAcceso from Usuarios where correo = ? and AES_DECRYPT(clave,?) = ?";
		try {
			if (null == preparedStatementBuscarPorCorreoClave) {
				preparedStatementBuscarPorCorreoClave = administradorDeConexionMariaDB.getConexion().prepareStatement(sql);
			}
